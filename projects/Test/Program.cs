...
namespace Test
{
    internal class Program
    {
        ...
        private static void Go(Engine engine)
        {
            ...
            var expMaxPlayerLevel = globalsDefinition.ExpMaxPlayerLevel;
            var expPointsRequiredForLevel = globalsDefinition.ExpPointsRequiredForLevel;

            var downloadableBalanceModifierDefinitionClass = engine.GetClass("WillowGame.DownloadableBalanceModifierDefinition");
            var downloadableBalanceModifierDefinitions = engine.Objects
                    .Where(o => o.IsA(downloadableBalanceModifierDefinitionClass) &&
                                o.GetName().StartsWith("Default__") ==
                                false)
                    .OrderBy(o => o.GetPath())
                    .ToArray();
            var downloadableBalanceModifierDefinition = downloadableBalanceModifierDefinitions[0];
